...
- 视觉视口就是用户可见的区域。绝对宽度永远与设备等宽，这个宽中的包含的css像素是变化的。 
- 获取大小
  ```js
    window.innerWidth
    window.innerHeight
...
- 设置理想视口的方法
  ...
  整理：pc端只有一个视口
  1. 在没有设备独立像素时，1px css像素等于1px 物理像素 此时只在pc端
  2. 出现设备独立像素时，1px css像素 对应 1px 设备独立像素 如果dpr=2 解析出对应2px 物理像素 
  问题:移动端要进行开发，怎么把pc的网页压缩到手机端
  1.  如果按照之前的写法，物理像素不可能在移动端显示完成。出现布局视口，默认为980px,当移动端设置css像素为980px->移动端默认对应375px独立像素->dpr=1->对应375px物理像素。
  2.  布局视口宽度===设备独立像素  相当于写出来的css像素，可以直接进行转换